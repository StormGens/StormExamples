...
import static com.example.stormcamera.Util.Assert;
 ...
 * 
 * <p>这个类用来hold一个相机实例
 *<p>{@code open()} and {@code release()}方法和{@code android.hardware.Camera}中的方法类似，
 *区别是如果在调用{@code release()}之前调用了{@code keep()}，那么CameraHolder将保持{@code
 * android.hardware.Camera}的一个实例一段时间，所以如果之后马上调用了{@code open()}，我们可以避免
 * {@codeandroid.hardware.Camera}中的{@code open()}带来的速度消耗。
 * <p>这是用在切换相机拍照和摄像间的。
